...
from PyQt5.QtWidgets import QApplication, QMainWindow, QWidget, QGridLayout, QFileDialog, QLabel, QMenuBar, QAction, QDockWidget, QLineEdit, QVBoxLayout
...
class MainWindow(QMainWindow):
    ...
    def __init__(self):
        ...
        self.playback_dock_widget.setStyleSheet(f'background-image: url({assets["checker_background"]}); border: 1px solid black;')
        ...
        # TODO: Add zoom function to this widget so it doesn't force the size of the main window to the size of the image in the widget, The image must also be centered with the widget.
        # Set up the sprite sheet dock widget
        self.sprite_sheet_dock_widget = QDockWidget("Sprite Sheet", self)
        self.sprite_sheet_dock_widget.setStyleSheet(f'background-image: url({assets["checker_background"]}); border: 1px solid black;')
        self.layout.addWidget(self.sprite_sheet_dock_widget, 1, 6, 9, 2)  # Adjust the row and column spans as needed

        # Set up the sprite sheet widget
