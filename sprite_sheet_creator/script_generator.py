...
public class SpriteSheetAnimator : MonoBehaviour
{
    ...
    def generate_lsl_script_option_1(self) -> str:
        ...
        """
        script = """integer link_number = 0;//Number representing the link number that this animation will be played on.
...
float start_frame = 0.0; //Frame to start animation on. (0 to start at the first frame of the texture)
float end_frame = 64.0; //Number of frames to animate, set to 0 to animate all frames.
float fps = 24.0; //Frames per second to play.

default
{
    state_entry()
    {
        // Stop animated texture.
        llSetLinkTextureAnim(link_number, FALSE, face, 0, 0, 0.0, 0.0, 1.0);
        
        // Loop a single sprite sheet.
        llSetLinkTextureAnim(link_number, ANIM_ON | LOOP, face, sideX, sideY, start_frame, end_frame, fps );
    }
}       
        """
        return script
